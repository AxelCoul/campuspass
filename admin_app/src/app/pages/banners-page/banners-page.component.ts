...
/** Gestion du bandeau sous le carrousel (position API `HOME_TOP`) — 2e section accueil app. */
@Component({
  selector: 'app-banners-page',
  standalone: true,
  imports: [CommonModule, FormsModule, DecimalPipe],
  templateUrl: './banners-page.component.html',
  styleUrls: ['./banners-page.component.css', '../advertisements-page/advertisements-page.component.css']
})
export class BannersPageComponent implements OnInit, OnDestroy {
  ...
  private readonly positionHomeTop = 'HOME_TOP' as const;
  ...
  loadAds(): void {
    this.loading = true;
    this.adsService.getAll({ position: this.positionHomeTop }).subscribe({
  ...
  deleteAd(a: Advertisement): void {
    if (!confirm(`Supprimer la bannière « ${a.title || a.id} » ?`)) return;
